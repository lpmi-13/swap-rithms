...
default_rootfs_image_repo="ghcr.io/lpmi-13/swap-rithms-rootfs"
default_first_party_image_tag="v2"
...
build_context="$(mktemp -d /tmp/swap-rithms-rootfs-build.XXXXXX)"
trap 'rm -rf "${build_context}"' EXIT

mkdir -p "${build_context}/swap-rithms" "${build_context}/playground/iximiuz"
...
cp "${repo_root}/go.mod" "${build_context}/swap-rithms/go.mod"
cp "${repo_root}"/*.go "${build_context}/swap-rithms/"
copy_path "${repo_root}/workers" "${build_context}/swap-rithms/workers"
copy_path "${repo_root}/README.md" "${build_context}/swap-rithms/README.md"
